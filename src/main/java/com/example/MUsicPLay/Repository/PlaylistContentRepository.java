...
public interface PlaylistContentRepository extends JpaRepository<PlaylistContent,Long> {
    ...
    List<Object[]> getAllByMood(@Param("user_id") Long user_id);
    ...
    @Query(value = "SELECT pc.*, c.*, p.* FROM playlist_content pc INNER JOIN content c ON pc.content_id = c.content_id INNER JOIN playlist p " +
            "ON p.playlist_id = pc.playlist_id WHERE p.name = :name AND p.user_id = :user_id and p.playlist_id = :playlist_id  " +
            "GROUP BY pc.playlist_content_id, id_page ORDER BY pc.playlist_content_id ASC", nativeQuery = true)
    List<PlaylistContent> getPlaylistContentById(@Param("name") String name, @Param("user_id") Long user_id, @Param("playlist_id") Long playlist_id);

    @Query(value = "SELECT pc.*, c.*, p.* FROM playlist_content pc INNER JOIN content c ON pc.content_id = c.content_id INNER JOIN playlist p " +
            "ON p.playlist_id = pc.playlist_id WHERE p.name = :name AND p.user_id = :user_id and p.playlist_id = :playlist_id  " +
            "GROUP BY pc.playlist_content_id, id_page ORDER BY pc.playlist_content_id ASC LIMIT 1", nativeQuery = true)
    List<PlaylistContent> getPlaylistContentById2(@Param("name") String name, @Param("user_id") Long user_id, @Param("playlist_id") Long playlist_id);
}